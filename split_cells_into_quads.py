## \file split_cells_into_quads.py
...
#  - Splits each mesh edge at midpoint.
#  - Splits each cell into quadrilaterals.
...
class VERTEX(HMESH_VERTEX_BASE):

    def __init__(self):

        super(VERTEX,self).__init__()

        # Initialize flag_old to False.
        flag_old = False
...
def main(argv):
    ...
    # Note: Use local class VERTEX, not HMESH_VERTEX_BASE.
    mesh = HALF_EDGE_MESH_BASE\
        (VERTEX,HMESH_HALF_EDGE_BASE,HMESH_CELL_BASE)
    ...
    for half_edge in edge_list:
        ...
        vsplit = mesh.SplitEdge(ihalf_edge)
        # Mark vsplit as new.
        vsplit.flag_old = False

    # Split cells into quads.
    icell_list = list(mesh.CellIndices())

    num_split = 0
    for icell in icell_list:
        cell = mesh.Cell(icell)
        # All cells should have even number of vertices, but just in case...
        if (cell.NumVertices()%2 == 0):
            if (flag_verbose):
                print(f"Splitting cell {str(icell)}.")

            half_edge = cell.HalfEdge()
            if (half_edge.FromVertex().flag_old == True):
                half_edge = half_edge.NextHalfEdgeInCell()
            mesh.SplitCellIntoQuads(half_edge.Index())
            num_split = num_split+1
        else:
            # Should never reach here, but just in case.
            print(f"Error in cell {icell}. Cell has odd number of vertices.")
            print(f"  Cell {icell} has {cell.NumVertices()}.")
            ivlist = cell.GetVertexIndices()
            print(f"  Cell vertices: {ivlist}")
            print(f"  Skipping split of cell {icell}.")

    if (not flag_silent):
        print(f"Split {num_split} cells into quadrilaterals.")

    # Check mesh after splitting cells.
...
def check_mesh_after_split\
    ...
    if not flag:
        sys.stderr.write\
            ("Error detected in mesh data structure after splitting cells.\n")
...
def usage_msg(out):
    out.write("Usage: python3 split_cells_into_quads.py [-s] [-no_warn] [-time] [-h] <input filename> [<output_filename>]")
...
def help():
    ...
    print("\n\nsplit_cells_into_quads.py -- Split cells into quadrilaterals.")
    print("  Read mesh from a .off file into HALF_EDGE_MESH data structure,")
    print("  split all edges at midpoint, split all cells into quads,")
